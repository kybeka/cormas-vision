...
2. **JSON file-passing** (the proposal's original idea). **Python side done** — `../05-runtime/json_bridge.py` + `python demo.py --json-out <dir>` atomically writes `<session>.json` each frame (see `PROTOCOL.md`). The Pharo poller that reads it is still to build. Simpler, no live socket, easier to debug.
